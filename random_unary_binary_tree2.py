...
leaves = np.array(["x", "-5", "-4", "-3", "-2", "-1", "1", "2", "3", "4", "5"])
p_leaf = np.array([1/6, 1/12, 1/12, 1/12, 1/12, 1/12, 1/12, 1/12, 1/12, 1/12, 1/12])
binary_operators = np.array(["+", "-", "*", "/"])
unary_operators = np.array(["exp", "log", "sqrt", "sin", "cos", "tan", "asin", "acos", "atan", "sinh", "cosh", "tanh", "asinh", "acosh", "atanh"])
...
def random_binary_trees(n):
    ...
    k, a = choice(population, p=weights)[0]
    if a == 1:
        operator = choice(unary_operators)[0]
        node = Node_Unary(operator)
        empty_node = [(node, 'middle')]
        e = 1
    else:
        operator = choice(binary_operators)[0]
    ...
    while n > 0:
        ...
        for ele in empty_node:
            if i < k:
                leave = choice(leaves, p=p_leaf)[0]
                value = Leaf(leave)
